...
public class UserServiceImpl implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        if (null == tempUser) {
            throw new UsernameNotFoundException(MessageFormat.format(ConstantMessages.USERNAME_NOT_FOUND, username));
        }
        User user = JSON.parseObject(tempUser.toString(), User.class);
        if (null == user) {
            throw new UsernameNotFoundException(MessageFormat.format(ConstantMessages.USERNAME_FOUND_BUT_PARSEERROR, username));
        }
        return user;
    }
}
